...
void fb_start(lua_State *L) {
  ...
  pushctuple(L, "copy", l_fbcopy);
  pushctuple(L, "fill", l_fbfill);
  pushctuple(L, "isReady", l_fb_ready);
  pushctuple(L, "getBackground", l_fb_getbg);
  pushctuple(L, "getForeground", l_fb_getfg);
  pushctuple(L, "get", l_fb_get);
  pushctuple(L, "getNearest", l_get_nearest);
